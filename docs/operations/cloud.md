...
| Camera vision preferences, observations and request receipts | Owner preferences and encrypted textual observations in PostgreSQL, with conversation deletion cascading to observations and receipts. One unpublished change per conversation is retained for comparison; published observations form the visible history. Encrypted request replay results expire after 24 hours and are cleared lazily on a later camera request; request UUID tombstones persist until conversation deletion. Camera JPEGs are memory-only. Explicitly attached captures follow ordinary message attachment retention. |
